...
# ===== CẤU HÌNH =====
KEY_DUNG = "NamDev"   # 🔑 đổi key tại đây
SERVER_URL = "https://device-server-340k.onrender.com/check_device"

# ===== TẠO HASH KEY =====
HASH_DUNG = hashlib.sha256(KEY_DUNG.encode()).hexdigest()

# ===== KIỂM TRA KEY =====
nhap = input("Nhập KEY để chạy: ").strip()
if hashlib.sha256(nhap.encode()).hexdigest() != HASH_DUNG:
    print("❌ KEY sai! Không thể chạy.")
    sys.exit()

print("✅ KEY đúng, đang khởi động...")

# ===== LẤY TÊN THIẾT BỊ (ĐẸP & RÕ) =====
def get_device_name():
    try:
        ...
        # Android / Linux / Windows / macOS đều ok
        return f"{hostname} ({system} {release})"
    except:
...
def send_device():
    ...
    print("➡️ Đang gửi lên server:", SERVER_URL)
    print("🖥️ Thiết bị:", device_name)
    ...
    if r.status_code == 403:
        print("⛔ Thiết bị này đã bị CHẶN")
        sys.exit()

    if r.status_code != 200:
        print("⚠️ Server phản hồi lỗi")
        sys.exit()

    print("✅ Thiết bị đã được ghi nhận")

# ===== CHẠY =====
send_device()

print("🚀 Tool đang chạy bình thường...")
# ---- PHẦN CODE CHÍNH CỦA BẠN ĐẶT DƯỚI DÒNG NÀY ----
